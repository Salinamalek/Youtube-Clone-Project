export default function DevsAbout() {
  return (
    <section>
      <div className="salina">
        <h1>Salina Malek</h1>
        <p>
          Hello world! My name is Salina (pronounced as Selena or 'suh-lee-na')
          and I am a student at Pursuit studying to become a fullstack web
          developer that positively impacts the world! As someone who enjoys
          volunteering and helping others, I hope to use the skills and
          knowledge obtained through Pursuit to develop applications that assist
          people with the means to improve their health, lifestyle, and quality
          of life with my shift into tech.
        </p>
        <a
          href="https://github.com/Salinamalek"
          target="_blank"
          rel="noopener noreferrer"
        >
          Salina's GitHub
        </a>
      </div>
      <div className="daniel">
        <h1>Daniel Rodriguez</h1>
        <p>
          Hello, my name is Daniel Rodriguez and I am a Full Stack Web
          Development Fellow at the Pursuit Fellowship based out of New York
          City. The Pursuit Fellowship is an intensive 12 month software
          engineering fellowship with a 9% acceptance rate and a project-based
          curriculum which provides experience with languages such as
          Javascript, CSS, and HTML.
        </p>
        <a
          href="https://github.com/DanielR2105"
          target="_blank"
          rel="noopener noreferrer"
        >
          Daniel's GitHub
        </a>
      </div>
    </section>
  );
}
